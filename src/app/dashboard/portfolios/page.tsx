...
import { AddForm } from './[id]/add-form';
		...
		<Grid container spacing={2} rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
			{portfolios.map((portfolio) => (
				<Grid key={portfolio.id} xs={2} md={2}>
					<Card key={portfolio.id} id={portfolio.id} title={portfolio.title} description={portfolio.description} imageSrc={portfolio.image_1_src} />
				</Grid>
			))}
		</Grid>
	)
